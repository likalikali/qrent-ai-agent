...
def show_consultation_stage():
    ...
    query = st.text_area(
        "请描述您的具体需求（可选）：",
        height=100,
        placeholder="例如：我想在UNSW附近找符合我预算的两室公寓，请推荐具体的区域和房源...\n\n💡 提示：如果您已完成前面的问卷和评估，可以不填写内容直接获取推荐，系统会根据您的需求自动生成建议。"
    )
    
    col_btn1, col_btn2 = st.columns(2)
    with col_btn1:
        if st.button("🔍 获取房源推荐", type="primary"):
            # 如果没有输入具体查询，则使用基于上下文的默认查询
            if not query.strip():
                # 构建基于用户需求的默认查询
                default_query = "请根据我的问卷信息和需求评估结果，为我推荐合适的房源和区域。"
                
                # 如果有具体的需求信息，添加到查询中
                if st.session_state.inquiry_agent:
                    updated_requirements = st.session_state.inquiry_agent.get_updated_requirements()
                    if updated_requirements.get('budget_min') and updated_requirements.get('budget_max'):
                        budget_info = f"预算{updated_requirements['budget_min']}-{updated_requirements['budget_max']}澳元/周"
                        default_query += f"我的{budget_info}，"
                    if updated_requirements.get('room_type'):
                        room_info = updated_requirements['room_type']
                        default_query += f"希望找{room_info}，"
                    if updated_requirements.get('commute_time'):
                        commute_info = updated_requirements['commute_time']
                        default_query += f"通勤时间要求{commute_info}。"
                elif st.session_state.questionnaire_data:
                    if st.session_state.questionnaire_data.get('budget_min') and st.session_state.questionnaire_data.get('budget_max'):
                        budget_info = f"预算{st.session_state.questionnaire_data['budget_min']}-{st.session_state.questionnaire_data['budget_max']}澳元/周"
                        default_query += f"我的{budget_info}，"
                    if st.session_state.questionnaire_data.get('room_type'):
                        room_info = st.session_state.questionnaire_data['room_type']
                        default_query += f"希望找{room_info}。"
                
                actual_query = default_query
            else:
                actual_query = query
            ...
            with st.spinner("正在搜索匹配的房源..."):
                try:
                    ...
                    # 使用主Agent进行房源搜索
                    result = st.session_state.agent.process_query(actual_query, top_k=5)
                    
                    # 显示搜索结果
                    st.markdown("### 🏠 房源推荐结果")
                    
                    # 如果使用了默认查询，显示实际使用的查询内容
                    if not query.strip():
                        st.info(f"💡 基于您的需求信息，系统自动生成查询：{actual_query}")
